...
//it saves the registred users here
const url = "mongodb://localhost:27017/travelPackage";
...
app.get('/', function(req, res) {
  ...
  //otherwise perfrom a search to return all the documents in the tpusers collection
  db.collection('tpusers').find().toArray(function(err, result) {
    if (err) throw err;
...
app.post('/dologin', function(req, res) {
  ...
  db.collection('tpusers').findOne({"login.username":uname}, function(err, result) {
...
app.post('/delete', function(req, res) {
  ...
  db.collection('tpusers').deleteOne({"login.username":uname}, function(err, result) {
    if (err) throw err;
...
app.post('/adduser', function(req, res) {
  //check we are logged in
  //HAD TO COMMENT THIS CODE BECAUSE IT SAYS AFTER LOGGED IN IT TAKES YOU BACK TO LOGI 
  //if(!req.session.loggedin){res.redirect('/login');return;}
...
//once created we just run the data string against the database and all our new data will be saved/
  db.collection('tpusers').save(datatostore, function(err, result) {
    if (err) throw err;
